/*@file Lazo_abierto.c
 * @brief Programa para controlar un motor DC con PWM y leer un encoder óptico.
 * @details Este programa genera una señal PWM para controlar la potencia suministrada a un motor DC.
 * Utiliza interrupciones para contar pulsos de un encoder óptico y calcular la velocidad del motor en RPM.
 * El programa reporta la velocidad cada 2 segundos y ajusta el duty cycle del PWM en un bucle.
 ...
 * @param events Eventos de interrupción (no se utiliza en este caso).
 */
 ...
 * 
 * Inicializa los pines GPIO, configura el PWM y el encoder, y entra en un bucle
 * donde ajusta el duty cycle del PWM y reporta la velocidad del motor en RPM.
 * 
 * @return int Código de salida del programa (0).
 */
int main() {
    stdio_init_all();
